...
#  因為使用的是較早期的 TensorFlow 語法，這行是為了確保每次跑腳本時計算圖都是乾淨的
...
print("\n=== Loading and Mixing Datasets 1 to 5 for Training and Validation ===\n") #==================================

x_train_list = [] #==================================
x_val_list = [] #==================================

# 混合 5 個資料集 (ds1 ~ ds5) 來訓練模型以符合題目 (c) 的要求
for i in range(1, 6): #==================================
    mat_train = sio.loadmat(f'data/DATA_Htrainin_ds{i}.mat') #==================================
    mat_val = sio.loadmat(f'data/DATA_Hvalin_ds{i}.mat') #==================================
    x_train_list.append(mat_train['HT'].astype('float32')) #==================================
    x_val_list.append(mat_val['HT'].astype('float32')) #==================================

# 將串列中的陣列沿著 batch 維度拼接起來
...
class LossHistory(Callback):
    ...
    # 為了把訓練過程的點點滴滴記錄下來，之後要畫 loss 曲線圖
...
file = 'CsiNet_'+(envir)+'_dim'+str(encoded_dim)+time.strftime('_%m_%d')+'_mixed' # 改個檔名方便區分 #==================================
...
def visualize_reconstruction(x_test, x_hat): #==================================
    #  圖畫在一起比一比。
...
def performance_evaluation(id): #==================================
    print(f"\n=== evaluation on Dataset{id} ===") #==================================

    mat = sio.loadmat(f'data/DATA_Htestin_ds{id}.mat') #==================================
    mathf = sio.loadmat(f'data/DATA_HtestFin_all_ds{id}.mat') #==================================
    ...
    # 神經網路的輸出是 0 到 1 的實數，所以我們要手動幫它把它們「組裝」回原本帶有實部和虛部的複數 (Complex)。
    ...
    filename = f"result/decoded_{file}_{id}.csv" #==================================
    x_hat1 = np.reshape(x_hat, (len(x_hat), -1))
    np.savetxt(filename, x_hat1, delimiter=",")

    filename = f"result/rho_{file}_{id}.csv" #==================================
    np.savetxt(filename, rho, delimiter=",")
    
    visualize_reconstruction(x_test, x_hat)  # Visualize original vs reconstructed CSI for first 10 samples #==================================

# 依序使用 5 個資料集的測試資料來評估泛化能力
for id in range(1, 6):
